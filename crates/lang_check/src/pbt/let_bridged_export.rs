...
fn assert_import_preserves_call(fun_text: &str, arg_text: &str) {
...
fn test_import_preserves_plain_lambda(tc: hegel::TestCase) {
    let (_body_ty, body_text) = tc.draw(primitive());
    let (_arg_ty, arg_text) = tc.draw(primitive());
...
#[hegel::test(test_cases = CASES)]
fn test_import_preserves_param_bridged_identity(tc: hegel::TestCase) {
    let (_arg_ty, arg_text) = tc.draw(primitive());
    assert_import_preserves_call("(__pbt_p: let __pbt_r = __pbt_p; in __pbt_r)", &arg_text);
}
...
#[hegel::test(test_cases = CASES)]
fn test_import_preserves_null_default_bridged(tc: hegel::TestCase) {
    let (_val_ty, val_text) = tc.draw(primitive());
    let arg_text = format!("{{ x = {val_text}; }}");
    assert_import_preserves_call("({ x ? null }: let __pbt_r = x; in __pbt_r)", &arg_text);
}
